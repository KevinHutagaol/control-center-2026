...
def add_tree(datas_list, src_dir, dest_prefix):
    ...
    for p in src_dir.rglob("*"):
        if p.is_file():
            # keep the directory structure under dest_prefix
...
# add folders (src -> dest)
add_tree(datas, home_ui,    os.path.join("pages", "Home",    "UI_home"))
add_tree(datas, home_asset, os.path.join("pages", "Home",    "Asset_home"))
...
# EXE: do NOT pass a.binaries / a.datas here for onedir
exe = EXE(
    pyz,
    a.scripts,
    [],
    [],                 # keep empty in onedir spec
    [],
    name='ControlCenter',
    debug=False,
